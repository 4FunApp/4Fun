...
public class PoemSpider {
	public void poemSpider() {
		try {
			PoemSpider poemSpider = new PoemSpider();
			Document document = JsoupUtil.connect(ConstantsUtil.POEM_BASE_URL
					+ "/mrhs/");
			...
			for (int i = 0; i < select.size(); i++) {
				Poem poem = poemSpider.parseDetail(ConstantsUtil.POEM_BASE_URL
	...
	private String insertSql(Poem poem) {
		return "insert into poemchina(poemContent,poemAuthor,poemDate,poemVoice,poemTitle) values('"
				+ poem.getPoemContent().replace("'", "\\\'")
				+ "','"
				+ poem.getPoemAuthor().replace("'", "\\\'")
				+ "','"
				+ poem.getPoemDate().replace("'", "\\\'")
				+ "','"
				+ poem.getPoemVoice().replace("'", "\\\'")
				+ "','"
	...
	private Poem parseDetail(String url) throws IOException {
		...
		// 推送时间 过滤中文
		poem.setPoemDate(info.get(1).text().replaceAll("[\u4e00-\u9fa5]", "-")
				.substring(0, 10));
		// 诗歌内容
		Elements contentElements = document.select("p > span");
		String content = "";
		// 有些网页可能是第二个 span 才是诗歌内容
		if ((content = contentElements.get(0).html().toString()
				.replace("<br style=\"box-sizing: border-box;\">", "\n"))
				.replace("&nbsp;", "").equals("")) {
			content = contentElements.get(1).html().toString()
					.replace("<br style=\"box-sizing: border-box;\">", "\n")
					.replace("&nbsp;", "");
		}
		...
		if (linkElements != null && linkElements.size() > 0) {
			link = ConstantsUtil.POEM_BASE_URL
					+ linkElements.get(0).attr("audio-url");
			poem.setPoemVoice(DownloadUtil.downloadRecitation(link,
